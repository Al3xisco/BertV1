...
top_k_reader = st.sidebar.slider("Nº Máx de Respuestas", min_value=1, max_value=10, value=3, step=1)
top_k_retriever = st.sidebar.slider("Nº Máx de Documentos a Consultar", min_value=1, max_value=10, value=3, step=1)


st.markdown('<h3>Pregunta</h3>', unsafe_allow_html=True)
question = st.text_input("Escriba su pregunta", value="What are the symptoms of COVID")
button = st.button('Obtener Respuesta')
st.text("")
st.text("")
